...
void handle_event(struct wiimote_t *wm) {
  ...
  /*
   *	Pressing minus will tell the wiimote we are no longer interested in
   *movement. This is useful because it saves battery power.
   */
  ...
  if (wm->exp.type == EXP_NUNCHUK || wm->exp.type == EXP_MOTION_PLUS_NUNCHUK) {
    ...
    printf("nunchuk joystick calibration (min, center, max): x: %i, %i, %i  y: "
           "%i, %i, %i\n",
           nc->js.min.x, nc->js.center.x, nc->js.max.x, nc->js.min.y,
           nc->js.center.y, nc->js.max.y);
  ...
  if (wm->exp.type == EXP_MOTION_PLUS ||
    ...
    printf("Motion+ angular rates (deg/sec): pitch:%03.2f roll:%03.2f "
           "yaw:%03.2f\n",
 ...
 *for no)
 *	@param ir				Is the IR support enabled? (1 for
 *yes, 0 for no)
