...
#define WIIUSE_VERSION_TRANSFORM(MAJ, MIN, MICRO)                              \
  (MAJ * 1000000 + MIN * 1000 + MICRO)
...
#define WIIMOTE_STATE_HANDSHAKE                                                \
  0x0002 /* actual connection exists but no handshake yet */
#define WIIMOTE_STATE_HANDSHAKE_COMPLETE                                       \
  0x0004 /* actual connection exists but no handshake yet */
...
#define WIIMOTE_STATE_EXP_HANDSHAKE                                            \
  0x10000 /* actual M+ connection exists but no handshake yet */
#define WIIMOTE_STATE_EXP_EXTERN                                               \
  0x20000 /* actual M+ connection exists but handshake failed */
#define WIIMOTE_STATE_EXP_FAILED                                               \
  0x40000 /* actual M+ connection exists but handshake failed */
...
#define EXP_MOTION_PLUS_CLASSIC                                                \
  7 /* Motion+ in classic ctr. pass-through mode */
/** @} */
 ...
 *	@param lvl		[out] Pointer to an int that will hold the level
 *setting. If no level is set 'lvl' will be set to 0.
 */
 ...
 *      library when the wiimote has returned the full data requested by a
 * previous call to wiiuse_read_data().
 */
...
typedef struct classic_ctrl_t {
  ...
  struct joystick_t
      rjs; /**< right joystick calibration				*/
} classic_ctrl_t;
...
typedef struct motion_plus_t {
  ...
  struct orient_t orient; /**< current orientation on each axis using Motion
                             Plus gyroscopes */
  byte acc_mode; /**< Fast/slow rotation mode for roll, pitch and yaw (0 if
                    rotating fast, 1 if slow or still) */
  int raw_gyro_threshold; /**< threshold for gyroscopes to generate an event */

  struct nunchuk_t
      *nc; /**< pointers to nunchuk & classic in pass-through-mode */
...
typedef struct wiimote_t {
  ...
  OVERLAPPED hid_overlap; /**< overlap handle
                           */
  enum win_bt_stack_t
      stack;           /**< type of bluetooth stack to use			*/
 ...
 *      A registered function of this type is called automatically by the wiiuse
 *      library when the wiimote has returned the full data requested by a
 * previous call to wiiuse_write_data().
 */
typedef void (*wiiuse_write_cb)(struct wiimote_t *wm, unsigned char *data,
                                unsigned short len);
...
struct data_req_t {
  ...
  byte len;
  unsigned int addr;
  data_req_s state; /**< set to 1 if not using callback and needs to be cleaned
                       up	*/
...
WIIUSE_EXPORT extern void wiiuse_set_output(enum wiiuse_loglevel loglevel,
                                            FILE *logtarget);
...
WIIUSE_EXPORT extern void wiiuse_motion_sensing(struct wiimote_t *wm,
                                                int status);
WIIUSE_EXPORT extern int wiiuse_read_data(struct wiimote_t *wm, byte *buffer,
                                          unsigned int offset, uint16_t len);
WIIUSE_EXPORT extern int wiiuse_write_data(struct wiimote_t *wm,
                                           unsigned int addr, const byte *data,
                                           byte len);
WIIUSE_EXPORT extern void wiiuse_status(struct wiimote_t *wm);
WIIUSE_EXPORT extern struct wiimote_t *wiiuse_get_by_id(struct wiimote_t **wm,
                                                        int wiimotes, int unid);
WIIUSE_EXPORT extern int wiiuse_set_flags(struct wiimote_t *wm, int enable,
                                          int disable);
WIIUSE_EXPORT extern float wiiuse_set_smooth_alpha(struct wiimote_t *wm,
                                                   float alpha);
WIIUSE_EXPORT extern void wiiuse_set_bluetooth_stack(struct wiimote_t **wm,
                                                     int wiimotes,
                                                     enum win_bt_stack_t type);
WIIUSE_EXPORT extern void wiiuse_set_orient_threshold(struct wiimote_t *wm,
                                                      float threshold);
WIIUSE_EXPORT extern void wiiuse_resync(struct wiimote_t *wm);
WIIUSE_EXPORT extern void wiiuse_set_timeout(struct wiimote_t **wm,
                                             int wiimotes, byte normal_timeout,
                                             byte exp_timeout);
WIIUSE_EXPORT extern void wiiuse_set_accel_threshold(struct wiimote_t *wm,
                                                     int threshold);
WIIUSE_EXPORT extern void
wiiuse_wiiboard_use_alternate_report(struct wiimote_t *wm, int enabled);

/* io.c */
WIIUSE_EXPORT extern int wiiuse_find(struct wiimote_t **wm, int max_wiimotes,
                                     int timeout);
 ...
 */
WIIUSE_EXPORT extern int wiiuse_update(struct wiimote_t **wm, int wiimotes,
                                       wiiuse_update_cb callback);

/* ir.c */
WIIUSE_EXPORT extern void wiiuse_set_ir(struct wiimote_t *wm, int status);
WIIUSE_EXPORT extern void wiiuse_set_ir_vres(struct wiimote_t *wm,
                                             unsigned int x, unsigned int y);
WIIUSE_EXPORT extern void wiiuse_set_ir_position(struct wiimote_t *wm,
                                                 enum ir_position_t pos);
WIIUSE_EXPORT extern void wiiuse_set_aspect_ratio(struct wiimote_t *wm,
                                                  enum aspect_t aspect);
WIIUSE_EXPORT extern void wiiuse_set_ir_sensitivity(struct wiimote_t *wm,
                                                    int level);
...
WIIUSE_EXPORT extern void wiiuse_set_motion_plus(struct wiimote_t *wm,
                                                 int status);
