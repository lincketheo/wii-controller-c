 ...
 * 00000000001 00101 000001 00 (major service - major class - minor class -
 * format type) This can also be seen in the WiiC Linux way: 00000000 00100101
 * 00000100
 */
#ifdef WIIUSE_MAC
...
#define WM_PLUS_DEV_MINOR_CLASS                                                \
  0x02 // For the newer RVL-CNT-01-TR (MotionPlus Inside)
...
#define WM_PLUS_DEV_CLASS_0                                                    \
  0x08 // For the newer RVL-CNT-01-TR (MotionPlus Inside)
...
#define WM_PRODUCT_ID_TR                                                       \
  0x0330 // RVL-CNT-01-TR Wiimote (with MotionPlus built-in)
...
#define EXP_ID_CODE_MOTION_PLUS_NUNCHUK                                        \
  0xA4200505 /** Motion Plus ID in Nunchuck passthrough mode */
#define EXP_ID_CODE_MOTION_PLUS_CLASSIC                                        \
  0xA4200705 /** Motion Plus ID in Classic control. passthrough */

/* decrypted M+ codes at 0x04A600FA */
#define EXP_ID_CODE_INACTIVE_MOTION_PLUS                                       \
  0xA6200005 /** Inactive Motion Plus ID */
#define EXP_ID_CODE_INACTIVE_MOTION_PLUS_BUILTIN                               \
  0xA4200005 /** Inactive Motion Plus ID in Wii Remote Plus                    \
              */
#define EXP_ID_CODE_NLA_MOTION_PLUS                                            \
  0xA6200405 /** No longer active Motion Plus ID */
#define EXP_ID_CODE_NLA_MOTION_PLUS_NUNCHUK                                    \
  0xA6200505 /** No longer active Motion Plus ID in Nunchuck passthrough mode  \
              */
#define EXP_ID_CODE_NLA_MOTION_PLUS_CLASSIC                                    \
  0xA6200705 /** No longer active Motion Plus ID in Classic control.           \
                passthrough */
...
#define WIIMOTE_TOGGLE_STATE(wm, s)                                            \
  ((wm->state & (s)) ? WIIMOTE_DISABLE_STATE(wm, s)                            \
                     : WIIMOTE_ENABLE_STATE(wm, s))
 ...
 *	Add 1 (Win32) or 2 (Mac, *nix) for the prefix and round up to a power
 *of 2.
 */
...
#define WIIUSE_VERSION                                                         \
  _STRINGIFY(WIIUSE_MAJOR)                                                     \
  "." _STRINGIFY(WIIUSE_MINOR) "." _STRINGIFY(WIIUSE_MICRO)
...
int wiiuse_read_data_cb(struct wiimote_t *wm, wiiuse_read_cb read_cb,
                        byte *buffer, unsigned int offset, uint16_t len);
...
/** @addtogroup bufferfunc Buffering functions
        @brief These wrap around from/to_big_endian_TYPE, but take a byte** so
   that they can advance the input/output pointer appropriately.
        @{
*/
/** @brief Converts the value val from system byte order to big endian,
