...
int wiiuse_os_find(struct wiimote_t **wm, int max_wiimotes, int timeout) {
  ...
  device_info = SetupDiGetClassDevs(&device_id, NULL, NULL,
  ...
  for (;; ++index) {
    ...
    if (!SetupDiEnumDeviceInterfaces(device_info, NULL, &device_id, index,
                                     &device_data)) {
      break;
    }

    /* get the size of the data block required */
    i = SetupDiGetDeviceInterfaceDetail(device_info, &device_data, NULL, 0,
                                        &len, NULL);
    detail_data = (SP_DEVICE_INTERFACE_DETAIL_DATA_A *)malloc(len);
    detail_data->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);

    /* query the data for this device */
    if (!SetupDiGetDeviceInterfaceDetail(device_info, &device_data, detail_data,
                                         len, NULL, NULL)) {
      continue;
    }
    ...
    if ((attr.VendorID == WM_VENDOR_ID) &&
        ((attr.ProductID == WM_PRODUCT_ID) ||
         (attr.ProductID == WM_PRODUCT_ID_TR))) {
      /* this is a wiimote */
...
int wiiuse_os_read(struct wiimote_t *wm, byte *buf, int len) {
  ...
  if (!ReadFile(wm->dev_handle, buf, len, &b, &wm->hid_overlap)) {
    ...
    if (r == WAIT_TIMEOUT) {
      ...
      /*
                  if (*buf) {
                          WIIUSE_WARNING("Packet ignored.  This may indicate a
         problem (timeout is %i ms).", wm->timeout);
                  }
      */
      ...
      WIIUSE_WARNING("A wait error occurred on reading from wiimote %i.",
                     wm->unid);
      return 0;
    }
...
int wiiuse_os_write(struct wiimote_t *wm, byte report_type, byte *buf,
                    int len) {
  DWORD bytes;
  int i;
  ...
  case WIIUSE_STACK_UNKNOWN: {
    ...
    if (i = WriteFile(wm->dev_handle, write_buffer, 22, &bytes,
                      &wm->hid_overlap)) {
  ...
  case WIIUSE_STACK_BLUESOLEIL:
    return WriteFile(wm->dev_handle, write_buffer, 22, &bytes,
                     &wm->hid_overlap);
  }
