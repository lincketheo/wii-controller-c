...
int wiiuse_os_find(struct wiimote_t **wm, int max_wiimotes, int timeout) {
  ...
  found_devices =
      hci_inquiry(device_id, timeout, 128, NULL, &scan_info, IREQ_CACHE_FLUSH);
  if (found_devices < 0) {
    perror("hci_inquiry");
    close(device_sock);
    return 0;
  ...
  for (i = 0; (i < found_devices) && (found_wiimotes < max_wiimotes); ++i) {
    ...
    if (is_wiimote_regular || is_wiimote_plus) {
      ...
      WIIUSE_INFO("Found wiimote (type: %s) (%s) [id %i].", str_type,
 ...
 *					If NULL, use the address in the struct set
 *by wiiuse_os_find().
 *
  ...
  for (i = 0; i < wiimotes; ++i) {
    ...
    if (FD_ISSET(wm[i]->in_sock, &fds)) {
      ...
      if (r > 0) {
        ...
        wm[i]->event =
            (r == 0) ? WIIUSE_DISCONNECT : WIIUSE_UNEXPECTED_DISCONNECT;
        evnt++;
        /* propagate the event:
...
int wiiuse_os_read(struct wiimote_t *wm, byte *buf, int len) {
  ...
  if (rc == -1) {
    ...
    if (errno == ENOTCONN) {
      ...
      WIIUSE_ERROR("Bluetooth appears to be disconnected. Wiimote unid %i will "
                   "be disconnected.",
                   wm->unid);
...
/* log the received data */
    ...
    if (buf[0] != 0x30) { /* hack for chatty Balance Boards that flood the logs
                             with useless button reports */
      int i;
