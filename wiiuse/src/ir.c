...
static int get_ir_sens(struct wiimote_t *wm, const byte **block1,
                       const byte **block2);
...
static int ir_correct_for_bounds(int *x, int *y, enum aspect_t aspect,
                                 int offset_x, int offset_y);
static void ir_convert_to_vres(int *x, int *y, enum aspect_t aspect, int vx,
                               int vy);

/* ir block data */
static const byte WM_IR_BLOCK1_LEVEL1[] =
    "\x02\x00\x00\x71\x01\x00\x64\x00\xfe";
static const byte WM_IR_BLOCK2_LEVEL1[] = "\xfd\x05";
static const byte WM_IR_BLOCK1_LEVEL2[] =
    "\x02\x00\x00\x71\x01\x00\x96\x00\xb4";
static const byte WM_IR_BLOCK2_LEVEL2[] = "\xb3\x04";
static const byte WM_IR_BLOCK1_LEVEL3[] =
    "\x02\x00\x00\x71\x01\x00\xaa\x00\x64";
static const byte WM_IR_BLOCK2_LEVEL3[] = "\x63\x03";
static const byte WM_IR_BLOCK1_LEVEL4[] =
    "\x02\x00\x00\x71\x01\x00\xc8\x00\x36";
static const byte WM_IR_BLOCK2_LEVEL4[] = "\x35\x03";
static const byte WM_IR_BLOCK1_LEVEL5[] =
    "\x07\x00\x00\x71\x01\x00\x72\x00\x20";
...
void wiiuse_set_ir(struct wiimote_t *wm, int status) {
  ...
  WIIUSE_DEBUG("Enabled IR camera for wiimote id %i (sensitivity level %i).",
               wm->unid, ir_level);
}
...
void wiiuse_set_ir_sensitivity(struct wiimote_t *wm, int level) {
  ...
  WIIMOTE_DISABLE_STATE(
      wm, (WIIMOTE_STATE_IR_SENS_LVL1 | WIIMOTE_STATE_IR_SENS_LVL2 |
...
static void interpret_ir_data(struct wiimote_t *wm) {
    ...
    if (wm->ir.state < 2) {
      ...
      for (i = 0; i < 4; ++i) {
        if (dot[i].visible) {
          ...
          ir_convert_to_vres(&wm->ir.x, &wm->ir.y, wm->ir.aspect,
      ...
      for (i = 0; i < 4; ++i) {
        if (dot[i].visible) {
          ...
          if (ir_correct_for_bounds(&x, &y, wm->ir.aspect, wm->ir.offset[0],
                                    wm->ir.offset[1])) {
            ir_convert_to_vres(&x, &y, wm->ir.aspect, wm->ir.vres[0],
                               wm->ir.vres[1]);
            wm->ir.x = x;
            wm->ir.y = y;
          }
...
#ifdef WITH_WIIUSE_DEBUG
  {
    ...
    for (i = 0; i < 4; ++i)
      if (dot[i].visible) {
        WIIUSE_DEBUG("IR[%i][order %i] (%.3i, %.3i) -> (%.3i, %.3i)", i,
                     dot[i].order, dot[i].rx, dot[i].ry, dot[i].x, dot[i].y);
      }
 ...
 *
 *	@param x		[out] The current X, it will be updated if
 *valid.
 *	@param y		[out] The current Y, it will be updated if
 *valid.
...
/**
 *	@brief Interpolate the point to the user defined virtual screen
 *resolution.
 */
static void ir_convert_to_vres(int *x, int *y, enum aspect_t aspect, int vx,
                               int vy) {
