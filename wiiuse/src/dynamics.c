 ...
 *	@param accel		[in] Pointer to a vec3b_t structure that holds
 *the raw acceleration data.
 *	@param orient		[out] Pointer to a orient_t structure that will
 *hold the orientation data.
 *	@param rorient		[out] Pointer to a orient_t structure that will
 *hold the non-smoothed orientation data.
 *	@param smooth		If smoothing should be performed on the angles
 *calculated. 1 to enable, 0 to disable.
 *
 ...
 */
void calculate_orientation(struct accel_t *ac, struct vec3b_t *accel,
                           struct orient_t *orient, int smooth) {
  float xg, yg, zg;
  float x, y, z;
 ...
 *	@param accel		[in] Pointer to a vec3b_t structure that holds
 *the raw acceleration data.
 *	@param gforce		[out] Pointer to a gforce_t structure that will
 *hold the gravity force data.
 */
...
static float applyCalibration(float inval, float minval, float maxval,
                              float centerval) {
  float ret;
  /* We don't use the exact ranges but the ranges + 1 in case we get bad
   * calibration data - avoid div0 */
  if (inval == centerval) {
    ret = 0;
...
void calc_joystick_state(struct joystick_t *js, float x, float y) {
   ...
   *	or less than the associated axis center value, it needs to be
   *interpolated between the center and the minimum or maxmimum rather than
   *between the minimum and maximum.
   *
   *	So we have something like this:
   *		(x min) [-1] ---------*------ [0] (x center) [0] -------- [1] (x
   *max) Where the * is the current x value. The range is therefore -1 to 1, 0
   *being the exact center rather than the middle of min and max.
   */
...
void apply_smoothing(struct accel_t *ac, struct orient_t *orient, int type) {
  switch (type) {
  case SMOOTH_ROLL: {
    /* it's possible last iteration was nan or inf, so set it to 0 if that
     * happened */
     ...
     */
    if (((ac->st_roll < 0) && (orient->roll > 0)) ||
        ((ac->st_roll > 0) && (orient->roll < 0))) {
  ...
  case SMOOTH_PITCH: {
    ...
    if (((ac->st_pitch < 0) && (orient->pitch > 0)) ||
        ((ac->st_pitch > 0) && (orient->pitch < 0))) {
