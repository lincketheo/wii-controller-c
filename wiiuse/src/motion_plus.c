...
void wiiuse_motion_plus_handshake(struct wiimote_t *wm, byte *data,
                                  unsigned short len) {
  uint32_t val;
  if (data == NULL) {
    wiiuse_read_data_cb(wm, wiiuse_motion_plus_handshake, wm->motion_plus_id,
                        WM_EXP_ID, 6);
  } else {
    ...
    WIIMOTE_ENABLE_STATE(
        wm,
        WIIMOTE_STATE_EXP); /* tell wiimote to include exp. data in reports */
...
void motion_plus_event(struct motion_plus_t *mp, int exp_type, byte *msg) {
  ...
  if (mp->ext == 0 || isMPFrame) { /* reading gyro frame */
    /* Check if the gyroscope is in fast or slow mode (0 if rotating fast, 1 if
     * slow or still) */
    mp->acc_mode =
        ((msg[4] & 0x2) << 1) | ((msg[3] & 0x1) << 1) | ((msg[3] & 0x2) >> 1);
    ...
    /* First calibration */
    if ((mp->raw_gyro.roll > 5000) && (mp->raw_gyro.pitch > 5000) &&
        (mp->raw_gyro.yaw > 5000) && (mp->raw_gyro.roll < 0x3fff) &&
        (mp->raw_gyro.pitch < 0x3fff) && (mp->raw_gyro.yaw < 0x3fff) &&
    ...
    if (exp_type == EXP_MOTION_PLUS_NUNCHUK) {
      ...
      calculate_orientation(&(mp->nc->accel_calib), &(mp->nc->accel),
                            &(mp->nc->orient),
                            NUNCHUK_IS_FLAG_SET(mp->nc, WIIUSE_SMOOTHING));

      calculate_gforce(&(mp->nc->accel_calib), &(mp->nc->accel),
                       &(mp->nc->gforce));
